...
//------------------------Parameter----------------------
`define TV_IN_coeffs "../tv/cdatafile/c.fir.autotvin_coeffs.dat"
parameter ADDR_WIDTH = 9;
parameter DATA_WIDTH = 32;
parameter coeffs_DEPTH = 128;
reg [31 : 0] coeffs_OPERATE_DEPTH = 0;
parameter coeffs_c_bitwidth = 16;
parameter START_ADDR = 0;
parameter fir_continue_addr = 0;
parameter fir_auto_start_addr = 0;
parameter coeffs_data_in_addr = 256;
...
reg  BREADY_reg = 0;
reg [DATA_WIDTH - 1 : 0] mem_coeffs [coeffs_DEPTH - 1 : 0] = '{default : 'h0};
reg [DATA_WIDTH-1 : 0] image_mem_coeffs [ (coeffs_c_bitwidth+DATA_WIDTH-1)/DATA_WIDTH * coeffs_DEPTH -1 : 0] = '{default : 'hz};
reg coeffs_write_data_finish;
...
//write coeffs reg
reg [31 : 0] write_coeffs_count = 0;
reg [31 : 0] coeffs_diff_count = 0;
reg write_coeffs_run_flag = 0;
reg write_one_coeffs_data_done = 0;
...
assign TRAN_control_write_data_finish = 1 & coeffs_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        write_coeffs_run_flag <= 0; 
        count_operate_depth_by_bitwidth_and_depth (coeffs_c_bitwidth, coeffs_DEPTH, coeffs_OPERATE_DEPTH);
    end
    else begin
        if (AESL_ready_reg === 1) begin
            write_coeffs_run_flag <= 1; 
        end
        else if ((write_one_coeffs_data_done == 1 && write_coeffs_count == coeffs_diff_count - 1) || coeffs_diff_count == 0) begin
            write_coeffs_run_flag <= 0; 
        end
    end
end
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        write_coeffs_count = 0;
    end
    else begin
        if (AESL_ready_reg === 1) begin
            write_coeffs_count = 0;
        end
        if (write_one_coeffs_data_done === 1) begin
            write_coeffs_count = write_coeffs_count + 1;
        end
    end
end
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        coeffs_write_data_finish <= 0;
    end
    else begin
        if (TRAN_control_start_in === 1) begin
            coeffs_write_data_finish <= 0;
        end
        if (write_coeffs_run_flag == 1 && write_coeffs_count == coeffs_diff_count) begin
            coeffs_write_data_finish <= 1;
        end
    end
end

initial begin : initial_diff_counter_coeffs
    ...
    integer j;
    integer k;
    reg [31 : 0] coeffs_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = coeffs_c_bitwidth;
    ...
    while (1) begin
        ...
        for (k = 0; k < coeffs_OPERATE_DEPTH; k = k + 1) begin
            for (i = 0; i < four_byte_num; i = i + 1) begin
                if (coeffs_c_bitwidth < 32) begin
                    coeffs_data_tmp_reg = mem_coeffs[k];
                end
                else begin
                    for (j = 0; j < 32; j = j + 1) begin
                        if (i*32 + j < coeffs_c_bitwidth) begin
                            coeffs_data_tmp_reg[j] = mem_coeffs[k][i*32 + j];
                        end
                        else begin
                            coeffs_data_tmp_reg[j] = 0;
                        end
                    end
                end
                if(image_mem_coeffs[k * four_byte_num  + i]!==coeffs_data_tmp_reg) begin
                coeffs_diff_count = coeffs_diff_count + 1;
                end
            end
        end
...
initial begin : write_coeffs
    integer write_coeffs_resp;
    integer process_num ;
    integer get_ack;
    integer four_byte_num;
    integer ceil_align_to_pow_of_two_four_byte_num;
    ...
    while (1) begin
        ...
        for (check_coeffs_count = 0; check_coeffs_count < coeffs_OPERATE_DEPTH; check_coeffs_count = check_coeffs_count + 1) begin
            if (ongoing_process_number === process_num && process_busy === 0 ) begin
                get_ack = 1;
                if (write_coeffs_run_flag === 1 && get_ack === 1) begin
                    process_busy = 1;
                    //write coeffs data 
                    for (i = 0; i < four_byte_num; i = i + 1) begin
                        if (coeffs_c_bitwidth < 32) begin
                            coeffs_data_tmp_reg = mem_coeffs[check_coeffs_count];
                        end
                        else begin
                            for (j = 0; j < 32; j = j + 1) begin
                                if (i*32 + j < coeffs_c_bitwidth) begin
                                    coeffs_data_tmp_reg[j] = mem_coeffs[check_coeffs_count][i*32 + j];
                                end
                                else begin
                                    coeffs_data_tmp_reg[j] = 0;
                                end
                        ...
                        if(image_mem_coeffs[check_coeffs_count * four_byte_num  + i]!==coeffs_data_tmp_reg) begin
                        write (coeffs_data_in_addr + check_coeffs_count * ceil_align_to_pow_of_two_four_byte_num * 4 + i * 4, coeffs_data_tmp_reg, write_coeffs_resp);
                        write_one_coeffs_data_done <= 1;
                        @(posedge clk);
                        write_one_coeffs_data_done <= 0;
                        image_mem_coeffs[check_coeffs_count * four_byte_num + i]=coeffs_data_tmp_reg;
                        end
                    end
                    process_busy = 0;
                end   
...
// Read data from file 
initial begin : read_coeffs_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [127 : 0] token; 
  reg [16 - 1 : 0] token_tmp; 
  //reg [coeffs_c_bitwidth - 1 : 0] token_tmp; 
  ...
  count_seperate_factor_by_bitwidth (coeffs_c_bitwidth , factor);
  fp = $fopen(`TV_IN_coeffs ,"rb"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_coeffs); 
      $finish; 
  end 
  ...
  while (trans_depth != 64'h5a5aa5a50f0ff0f0)  begin
      ...
      for(i = 0; i < coeffs_DEPTH; i = i + 1) begin 
         $fread(token_tmp,fp);
          if (factor == 4) begin
              if (i%factor == 0) begin
              ...
              if (i%factor == 3) begin
                  ...
                  mem_coeffs [i/factor] = tmp_cache_mem;
                  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin
